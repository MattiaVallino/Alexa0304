    ...
    async handle(handlerInput) {
        ...
        if(count_new_therapies < new_therapies.length) {
            let new_therapy_name  = new_therapies[count_new_therapies].drug.split("-")[0];
              const requestAttributes = handlerInput.attributesManager.getRequestAttributes();
            const speakOutput=requestAttributes.t('NEW_THERAPY_MSG',new_therapy_name);
        ...
        //manage Therapies that have therapy.edit='updated' to setup
            ...
            let updated_therapy_name  = updated_therapies[count_updated_therapies].drug.split("-")[0];
             const requestAttributes = handlerInput.attributesManager.getRequestAttributes();
            const speakOutput=requestAttributes.t('UPDATED_THERAPY_MSG',updated_therapy_name);
        ...
        // if all therapies 'new' and 'updated' are set up, inizialize count and exit from the intent
          ...
          console.log('rem  ' + sessionAttributes['reminders'])
            const requestAttributes = handlerInput.attributesManager.getRequestAttributes();
            const speakOutput=requestAttributes.t('REMINDERS_COMPLETED_MSG');
...
const WhichMedicineTodayIntent = {
    ...
    let list_therapies = sessionAttributes['all_therapies'];
          ...
          // avrei dovuto mettere come filtro missed o programmed ma è capitato che fosse programmed ma doveva essere missed
            ...
            //console.log('check  ' +  logic.convertDateFromDatabase(list_programmed_intakes[0].programmed_date).format('l'))
            ...
            console.log('ciao '+moment(list_programmed_intakes[0].programmed_date).isSameOrAfter(today,'hour'))
            console.log('ciao '+moment(list_programmed_intakes[0].programmed_date).isSameOrBefore(today,'hour'))
           let list_today_old_intakes = list_programmed_intakes.filter(intake => moment(intake.programmed_date).isSameOrBefore(today,'hour'))
            list_programmed_intakes = list_programmed_intakes.filter(intake => moment(intake.programmed_date).isSameOrAfter(today,'hour'))
            console.log(456)
            //= list_programmed_intakes.filter(intake => (logic.convertDateFromDatabase(intake.programmed_date).format('LT')).isSameOrBefore(today.format('LT')))
             
            console.log(453 + ' ' + JSON.stringify(list_programmed_intakes))
            console.log(453 + ' ' + JSON.stringify(list_today_old_intakes))
            
        let speakOutput;
        if (list_programmed_intakes.length===0){
          
             const requestAttributes = handlerInput.attributesManager.getRequestAttributes();
                const speakOutput=requestAttributes.t('NO_MEDICINE_MSG');
        }else{
...
const ConfirmIntakeIntentHandler = {
    ...
    if ( sessionAttributes['flag_therapies']===1){
        ...
        // to change with a certain frequency the message
        let counter_msg_intake=sessionAttributes['counter_msg_intake'];
        let msg_intake_max=3;
        sessionAttributes['msg_intake_max']=msg_intake_max
        if (counter_msg_intake===undefined || counter_msg_intake === msg_intake_max){
            
            counter_msg_intake=0;
        }
        
        console.log(counter_msg_intake)   
...
const WhichMedicineIntentHandler = {
   ...
   async handle(handlerInput) {
        ...
        const requestAttributes = handlerInput.attributesManager.getRequestAttributes();
         speakOutput=requestAttributes.t('TAKEN_CONFIRMATION_MSG',signed_medicine_MSG,medicine_posology,medicine_name);// FUNZIONA?
...
const YesIntentHandler = {
    ...
    speakOutput=`${JSON.stringify(reminders)}` // questi speakOutput sono da eliminare 
   ...
   await reminderApiClient.deleteReminder(alertToken)  
    ...
    /* 
     if (counter_msg_intake < sessionAttributes['msg_intake_max'])
     {
   ...
   sessionAttributes['reminders'] = reminders_ids_left;
        ...
        counter_msg_intake++;
        sessionAttributes['counter_msg_intake']=counter_msg_intake;
...
const LastIntakeIntentHandler = {
    ...
    handle(handlerInput) {
        ...
        let date_w = moment(therapy.programmed_date).tz(timezone)    
...
const RequestMissedIntakesIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        const requestAttributes = handlerInput.attributesManager.getRequestAttributes();
              const speakOutput=requestAttributes.t('FORGOTTEN_MSG',numberMissedIntake,phrasemissedintakes);
...
const RequestAdherenceIntentHandler = {
    ...
    async handle(handlerInput) {
        ...
        // TENGO?
        if (adherence < 30) {
            const speakOutput=requestAttributes.t('LOW_ADHERENCE',adherence);
        }
        else if (adherence < 80) {
            const speakOutput=requestAttributes.t('MID_ADHERENCE',adherence);
         }
        else if (adherence >= 80) {
            const speakOutput=requestAttributes.t('HIGH_ADHERENCE',adherence);
             }
        else {
            const speakOutput=requestAttributes.t('ERROR_ADHERENCE');
...
const HelpUserIntentHandler = {
   canHandle(handlerInput) {
    ...
    handle(handlerInput, error) {
        ...
        oppure alexa, qual è la mia aderenza, per sapere la tua percentuale di aderenza alle terapie, oppure alexa, cancella tutti i dati in sessione, per eliminare tutti \
